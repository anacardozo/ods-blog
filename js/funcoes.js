...
function noticiageral(){
   for(i=y.length-1;i>=0;i--){
    document.write(
        ...
        "</div>"+
        "<div class='card-body'>"+
        "<p class='card-text'>"+y[i].getElementsByTagName("texto")[0].childNodes[0].nodeValue.substr(0,400) + "..."+"</p>"+ "<a class='btn btn-primary' href='postagem.html?codigo_postagem="+i+"'class='text-white fw-bold'>Saiba mais</a>"+
         "</div>"+
         "</div>"+
        "</div>"+
        "</div>");
